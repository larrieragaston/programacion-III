/**
 * Genera PDF desde el HTML ya construido por VitePress (Asincronismo docs).
 * Puerto 4178 para no chocar con lambda-calculus-docs (4173), clojure-docs (4174),
 * git-github-docs (4175), js-funcional-docs (4176) ni js-contemporaneo-docs (4177).
 */
...
const asincronismoPdfDestDirs = [
  path.join(repoRoot, 'asincronismo/pdfs/originales'),
  path.join(repoRoot, 'asincronismo/public/pdfs/originales'),
]
const base =
  process.env.VP_BASE !== undefined && process.env.VP_BASE !== ''
    ? process.env.VP_BASE
    : process.env.CI === 'true'
      ? '/programacion-III/asincronismo-docs/'
      : '/'

const PRINT_PORT = 4178

/** Orden: apunte → guía de ejercicios (teórico antes que práctica). */
const pages = [
  ['apunte.html', 'asincronismo-apunte.pdf', 'Apunte teórico — Asincronismo'],
  ['ejercicios.html', 'asincronismo-ejercicios.pdf', 'Guía de ejercicios — Asincronismo'],
]
...
function copyPdfToAsincronismoSlidevPublic(pdfPath, pdfName) {
  for (const dir of asincronismoPdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToAsincronismoSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a asincronismo/.../pdfs/originales/')
