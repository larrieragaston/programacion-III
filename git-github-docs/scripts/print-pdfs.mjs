/**
 * Genera PDF desde el HTML ya construido por VitePress (Git & GitHub docs).
 * Puerto 4175 para no chocar con lambda-calculus-docs (4173) ni clojure-docs (4174).
 */
...
const gitGithubPdfDestDirs = [
  path.join(repoRoot, 'git-github/pdfs/originales'),
  path.join(repoRoot, 'git-github/public/pdfs/originales'),
]
const base =
  process.env.VP_BASE !== undefined && process.env.VP_BASE !== ''
    ? process.env.VP_BASE
    : process.env.CI === 'true'
      ? '/programacion-III/git-github-docs/'
      : '/'

const PRINT_PORT = 4175

/** Orden: apunte → guía de ejercicios (teórico antes que práctica). */
const pages = [
  ['apunte.html', 'git-github-apunte.pdf', 'Apunte teórico — Git & GitHub'],
  ['ejercicios.html', 'git-github-ejercicios.pdf', 'Guía de ejercicios — Git & GitHub'],
]
...
function copyPdfToGitGithubSlidevPublic(pdfPath, pdfName) {
  for (const dir of gitGithubPdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToGitGithubSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a git-github/.../pdfs/originales/')
