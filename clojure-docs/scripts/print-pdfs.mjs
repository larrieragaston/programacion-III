/**
 * Genera PDF desde el HTML ya construido por VitePress (Clojure docs).
 * Puerto 4174 para no chocar con lambda-calculus-docs (4173) si se corre en paralelo.
 */
...
const clojurePdfDestDirs = [
  path.join(repoRoot, 'clojure/pdfs/originales'),
  path.join(repoRoot, 'clojure/public/pdfs/originales'),
]
const base =
...
const PRINT_PORT = 4174

/** Orden: apunte → ejercicios iniciales → guía (teórico antes que prácticas). */
const pages = [
  ['apunte.html', 'clojure-apunte.pdf', 'Apunte teórico — Clojure'],
  [
    'ejercicios-iniciales.html',
    'clojure-ejercicios-iniciales.pdf',
    'Ejercicios iniciales — Clojure',
  ],
  ['ejercicios.html', 'clojure-ejercicios.pdf', 'Guía de ejercicios — Clojure'],
]
...
function copyPdfToClojureSlidevPublic(pdfPath, pdfName) {
  for (const dir of clojurePdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToClojureSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a clojure/.../pdfs/originales/')
