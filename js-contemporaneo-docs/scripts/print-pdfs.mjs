/**
 * Genera PDF desde el HTML ya construido por VitePress (JS Contemporáneo docs).
 * Puerto 4177 para no chocar con lambda-calculus-docs (4173), clojure-docs (4174),
 * git-github-docs (4175) ni js-funcional-docs (4176).
 */
...
const jsContemporaneoPdfDestDirs = [
  path.join(repoRoot, 'js-contemporaneo/pdfs/originales'),
  path.join(repoRoot, 'js-contemporaneo/public/pdfs/originales'),
]
const base =
  process.env.VP_BASE !== undefined && process.env.VP_BASE !== ''
    ? process.env.VP_BASE
    : process.env.CI === 'true'
      ? '/programacion-III/js-contemporaneo-docs/'
      : '/'

const PRINT_PORT = 4177

/** Orden: apunte → guía de ejercicios (teórico antes que práctica). */
const pages = [
  ['apunte.html', 'js-contemporaneo-apunte.pdf', 'Apunte teórico — JS Contemporáneo'],
  ['ejercicios.html', 'js-contemporaneo-ejercicios.pdf', 'Guía de ejercicios — JS Contemporáneo'],
]
...
function copyPdfToJsContemporaneoSlidevPublic(pdfPath, pdfName) {
  for (const dir of jsContemporaneoPdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToJsContemporaneoSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a js-contemporaneo/.../pdfs/originales/')
