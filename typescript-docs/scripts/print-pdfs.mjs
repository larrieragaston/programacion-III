/**
 * Genera PDF desde el HTML ya construido por VitePress (TypeScript docs).
 * Puerto 4179 para no chocar con lambda-calculus-docs (4173), clojure-docs (4174),
 * git-github-docs (4175), js-funcional-docs (4176), js-contemporaneo-docs (4177)
 * ni asincronismo-docs (4178).
 */
...
const typescriptPdfDestDirs = [
  path.join(repoRoot, 'typescript/pdfs/originales'),
  path.join(repoRoot, 'typescript/public/pdfs/originales'),
]
const base =
  process.env.VP_BASE !== undefined && process.env.VP_BASE !== ''
    ? process.env.VP_BASE
    : process.env.CI === 'true'
      ? '/programacion-III/typescript-docs/'
      : '/'

const PRINT_PORT = 4179

/** Orden: apunte → guía de ejercicios (teórico antes que práctica). */
const pages = [
  ['apunte.html', 'typescript-apunte.pdf', 'Apunte teórico — TypeScript'],
  ['ejercicios.html', 'typescript-ejercicios.pdf', 'Guía de ejercicios — TypeScript'],
]
...
function copyPdfToTypescriptSlidevPublic(pdfPath, pdfName) {
  for (const dir of typescriptPdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToTypescriptSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a typescript/.../pdfs/originales/')
