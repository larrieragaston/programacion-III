...
El deck Slidev hermano (`<slug>/`) usa el siguiente puerto libre en la secuencia `dev.sh` (3031, 3032, ... — ver array `PORTS` en `dev.sh`). **Nota**: Asincronismo (3038), TypeScript (3039) y React (3040) se construyeron en branches paralelos a partir de `main` (no secuenciales entre sí) — los puertos ya están asignados de antemano siguiendo el orden de esta tabla para que no choquen al mergear. Asincronismo y TypeScript ya están mergeados/en esta branch; React sigue en su propio branch paralelo. Revisar conflictos en `index.html`/`dev.sh`/`.gitignore`/este archivo al mergear los PRs restantes, en orden.
