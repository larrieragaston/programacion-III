...
- **Built (`data-category="unidad"`)**: Introducción (deck only), Cálculo λ, Clojure, Git & GitHub, JS Funcional, JS Contemporáneo, React (deck + docs each, except Introducción). Asincronismo and TypeScript were built in parallel feature branches — check `git log`/open PRs if this file is being read from a branch that predates their merge.
