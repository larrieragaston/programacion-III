/**
 * Genera PDF desde el HTML ya construido por VitePress (JS Funcional docs).
 * Puerto 4176 para no chocar con lambda-calculus-docs (4173), clojure-docs (4174)
 * ni git-github-docs (4175).
 */
...
const jsFuncionalPdfDestDirs = [
  path.join(repoRoot, 'js-funcional/pdfs/originales'),
  path.join(repoRoot, 'js-funcional/public/pdfs/originales'),
]
const base =
  process.env.VP_BASE !== undefined && process.env.VP_BASE !== ''
    ? process.env.VP_BASE
    : process.env.CI === 'true'
      ? '/programacion-III/js-funcional-docs/'
      : '/'

const PRINT_PORT = 4176

/** Orden: apunte → guía de ejercicios (teórico antes que práctica). */
const pages = [
  ['apunte.html', 'js-funcional-apunte.pdf', 'Apunte teórico — JS Funcional'],
  ['ejercicios.html', 'js-funcional-ejercicios.pdf', 'Guía de ejercicios — JS Funcional'],
]
...
function copyPdfToJsFuncionalSlidevPublic(pdfPath, pdfName) {
  for (const dir of jsFuncionalPdfDestDirs) {
...
for (const [html, pdfName, docTitle] of pages) {
  ...
  copyPdfToJsFuncionalSlidevPublic(outPath, pdfName)
}

await context.close()
await browser.close()
server.close()
console.error('PDFs en docs/public/pdfs/ y copiados a js-funcional/.../pdfs/originales/')
